...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String freezerID = sharedPreferences.getString("freezerId", "-1");
    ...
    public void login(View view) {
        ...
        if (R.id.connectToExistingFreezer == radioGroup.getCheckedRadioButtonId()) {
            ...
            sharedPreferences.edit().putString("freezerId", uniqueFreezerID).apply();
            isValid = true;
        }
        ...
        FirebaseDatabase.getInstance().getReference().child(freezerCodeET.getText().toString().trim()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChildren()) {
                    sharedPreferences.edit().putString("freezerId", freezerCodeET.getText().toString().trim()).apply();
